...
// ── LOAD PRODUCTS (both products + promotions) ────────────────
function loadSellerProducts(uid) {
  const qProducts   = query(collection(db, "products"),   where("sellerUid", "==", uid));
  const qPromotions = query(collection(db, "promotions"), where("sellerUid", "==", uid));
  ...
  onSnapshot(qProducts,   (snap) => {
    products = snap.docs.map(d => ({ id: d.id, _collection: "products",   ...d.data() }));
    merge();
  });

  onSnapshot(qPromotions, (snap) => {
    promotions = snap.docs.map(d => ({ id: d.id, _collection: "promotions", ...d.data() }));
    merge();
  });
}
...
function renderProductTable(products) {
  ...
  tbody.innerHTML = products.map(p => `
    <tr>
      ...
      <td>
        <span class="badge ${p.category === 'promo' ? 'badge-promo' : ''}"
          style="${p.category !== 'promo' ? 'background:rgba(255,255,255,0.06);color:var(--muted);' : ''}">
          ${p.category || '—'}
        </span>
      </td>
...
function renderRecentProducts(products) {
  ...
  tbody.innerHTML = recent.map(p => `
    <tr>
      <td>${p.imageURL && p.imageURL !== "images/no-image.png"
        ? `<img src="${p.imageURL}" class="product-thumb">`
        : `<div class="product-thumb-placeholder"><i class="fas fa-image"></i></div>`}</td>
      <td style="font-weight:500;">${p.name}</td>
...
window.saveEditProduct = async () => {
  ...
  // ✅ Use the stored _collection to know which collection to update
  const collectionName = product._collection || (product.category === "promo" ? "promotions" : "products");
...
window.confirmDelete = async (id) => {
  ...
  // ✅ Use the stored _collection to know which collection to delete from
  const collectionName = product._collection || (product.category === "promo" ? "promotions" : "products");

  try {
    await deleteDoc(doc(db, collectionName, id));
    showMsg("sProductMsg", "Product deleted ✅", true);
  } catch (err) {
